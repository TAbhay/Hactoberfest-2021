# More Info Here https://github.com/aws-samples/aws-media-services-vod-automation
...
def lambda_handler(event, context):
    ...
    try:
        ...
        client = boto3.client(
            "mediaconvert", region_name=region, endpoint_url=endpoints["Endpoints"][0]["Url"], verify=False
        )
        ...
        for j in jobs:
            ...
            for outputGroup in jobSettings["OutputGroups"]:
                ...
                logger.info(
                    "outputGroup['OutputGroupSettings']['Type'] == %s", outputGroup["OutputGroupSettings"]["Type"]
                )
                ...
                if outputGroup["OutputGroupSettings"]["Type"] == "HLS_GROUP_SETTINGS":
                    ...
                    outputGroup["OutputGroupSettings"]["HlsGroupSettings"]["Destination"] = (
